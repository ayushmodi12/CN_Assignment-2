...
def run():
    ...
    # Adding route for ra-rc -- For ra->rc  Uncomment the belo line 
    info(net['ra'].cmd("ip route add 10.2.0.0/24 via 10.250.0.1 dev ra-eth3"))

    #For ra->rb->rc    -- Uncomment the belo line 
    #info(net['ra'].cmd("ip route add 10.2.0.0/24 via 10.100.0.2 dev ra-eth2"))
    ...
    # Adding route for rc-ra -- For ra->rc  Uncomment the belo line 
    ...
    #For ra->rb->rc -- Uncomment the below line 
    #info(net['rc'].cmd("ip route add 10.0.0.0/24 via 10.200.0.1 dev rc-eth2"))

    net.start()
    info('*** Measuring latency using ping:\n')
    ping_output = net['h1'].cmd('ping -c 5 {}'.format(net['h6'].IP()))
    info('Ping Latency Measurement:\n')
    info(ping_output)

    traceroute_output = net['h1'].cmd('traceroute {}'.format(net['h6'].IP()))
    info('Traceroute Path:\n')
    info(traceroute_output)

    info('*** Measuring latency using iperf:\n')
    net['h6'].cmd('iperf -s &')  # Start iperf server on h6
    time.sleep(1)  
    iperf_output = net['h1'].cmd('iperf -c {} -t 5'.format(net['h6'].IP()))
    info('Iperf Latency Measurement :\n')
    info(iperf_output)
